...
import GridLayout, { CardConfig } from './layout/GridLayout';
...
export default function TeamCommandCenter({
  ...
  // ── Layout cards ─────────────────────────────────────────────────────────────
  const tccCards = useMemo<CardConfig[]>(() => {
    const base: CardConfig[] = [
      { id: 'tcc-kpi-hero', label: 'KPIs Principais',        defaultColSpan: 12 },
      { id: 'tcc-priority', label: 'Tarefas por Prioridade', defaultColSpan: 8 },
      { id: 'tcc-sidebar',  label: 'Destaques & Timeline',   defaultColSpan: 4 },
    ];
    if (children) base.push({ id: 'tcc-extra', label: 'Conteúdo Extra', defaultColSpan: 12 });
    return base;
  }, [!!children]); // eslint-disable-line react-hooks/exhaustive-deps

  function renderCard(id: string) {
    if (id === 'tcc-kpi-hero') return (
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(12, 1fr)', gap: 14 }}>
        ...
        <div style={{
          ...
          <div style={{ fontSize: '.6rem', fontWeight: 700, color: 'rgba(238,242,248,.22)', textTransform: 'uppercase', letterSpacing: '.12em', marginBottom: 12 }}>Concluídas</div>
          {loading ? <Skel w="55%" h={44} r={10} /> : (
            <>
              <div style={{ fontSize: '2.8rem', fontWeight: 800, letterSpacing: '-.05em', color: completionPct > 0 ? '#4ADE80' : 'rgba(238,242,248,.25)', lineHeight: 1 }}>{closed.length}</div>
              <div style={{ fontSize: '.67rem', color: 'rgba(238,242,248,.28)', marginTop: 5 }}>{completionPct}% do total · {tasks.length} tarefas</div>
        ...
        <div style={{
          ...
          <div style={{ fontSize: '.6rem', fontWeight: 700, color: 'rgba(238,242,248,.22)', textTransform: 'uppercase', letterSpacing: '.12em', marginBottom: 12 }}>Em andamento</div>
          {loading ? <Skel w="55%" h={44} r={10} /> : (
            <>
              <div style={{ fontSize: '2.8rem', fontWeight: 800, letterSpacing: '-.05em', color: doingTasks.length > 0 ? '#3D7BFF' : 'rgba(238,242,248,.25)', lineHeight: 1 }}>{doingTasks.length}</div>
        ...
        <div style={{
          ...
          <div style={{ fontSize: '.6rem', fontWeight: 700, color: 'rgba(238,242,248,.22)', textTransform: 'uppercase', letterSpacing: '.12em', marginBottom: 12 }}>Alta prioridade</div>
          {loading ? <Skel w="55%" h={44} r={10} /> : (
            <>
              <div style={{ fontSize: '2.8rem', fontWeight: 800, letterSpacing: '-.05em', color: hasUrgent ? '#FF6B6B' : 'rgba(238,242,248,.25)', lineHeight: 1 }}>{highPriority.length}</div>
              {highPriority.length > 0 ? (
                <div style={{ marginTop: 8, display: 'flex', flexDirection: 'column', gap: 3 }}>
                  {highPriority.slice(0, 3).map(t => (
                    <div key={t.id} style={{ fontSize: '.63rem', color: 'rgba(238,242,248,.45)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', paddingLeft: 8, borderLeft: '2px solid rgba(255,107,107,.35)' }}>{t.name}</div>
                  ))}
                </div>
              ) : (
        ...
        <div style={{
          ...
          <div style={{ fontSize: '.6rem', fontWeight: 700, color: 'rgba(238,242,248,.22)', textTransform: 'uppercase', letterSpacing: '.12em', marginBottom: 12 }}>Vence hoje</div>
          {loading ? <Skel w="55%" h={44} r={10} /> : (
            <>
              <div style={{ fontSize: '2.8rem', fontWeight: 800, letterSpacing: '-.05em', color: dueToday.length > 0 ? '#FBBF24' : 'rgba(238,242,248,.25)', lineHeight: 1 }}>{dueToday.length}</div>
    ...
    if (id === 'tcc-priority') return (
      <div style={{ background: '#0B0D1A', borderRadius: 20, padding: '24px 26px', border: '1px solid rgba(255,255,255,.05)', height: '100%' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20, flexWrap: 'wrap', gap: 8 }}>
          <span style={{ fontSize: '.875rem', fontWeight: 700, color: '#EEF2F8' }}>Tarefas por prioridade</span>
          <div style={{ display: 'flex', gap: 6 }}>
            {!loading && [
              { label: `🔴 ${highPriority.length}` },
              { label: `🟡 ${normalPriority.length}` },
              { label: `⚪ ${lowPriority.length}` },
            ].map(b => (
        ...
        {loading ? (
          ...
          <>
            <PrioritySection title="Alta prioridade" tasks={highPriority} accentColor="#FF6B6B" badgeColor="#FF6B6B" badgeBg="rgba(255,107,107,.12)" />
            <PrioritySection title="Média prioridade" tasks={normalPriority} accentColor="#FBBF24" badgeColor="#FBBF24" badgeBg="rgba(251,191,36,.12)" />
            <PrioritySection title="Baixa / sem prioridade" tasks={lowPriority} accentColor="rgba(238,242,248,.18)" badgeColor="rgba(238,242,248,.38)" badgeBg="rgba(255,255,255,.06)" />
          </>
        )}
      </div>
    );

    if (id === 'tcc-sidebar') return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 14, height: '100%' }}>
        {/* Próximos prazos */}
        <div style={{ background: '#0B0D1A', borderRadius: 20, padding: '20px 22px', border: '1px solid rgba(255,255,255,.05)' }}>
          <div style={{ fontSize: '.6rem', fontWeight: 700, color: 'rgba(238,242,248,.22)', textTransform: 'uppercase', letterSpacing: '.12em', marginBottom: 14 }}>Próximos prazos</div>
          {/* content follows below */}
          {loading ? (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              {[1,2,3].map(i => <Skel key={i} w="100%" h={34} r={10} />)}
            </div>
          ) : upcoming.length === 0 ? (
            <div style={{ fontSize: '.72rem', color: 'rgba(238,242,248,.2)', padding: '4px 0' }}>Sem prazos próximos</div>
          ) : (
              ...
              {upcoming.map(t => {
                ...
                return (
                  <a key={t.id} href={t.url} target="_blank" rel="noopener noreferrer" style={{ textDecoration: 'none' }}>
                    <div
                      style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 10px', borderRadius: 10, background: 'rgba(255,255,255,.025)', border: `1px solid ${isToday ? 'rgba(251,191,36,.2)' : 'rgba(255,255,255,.04)'}`, transition: 'background .15s' }}
                      onMouseEnter={e => ((e.currentTarget as HTMLElement).style.background = 'rgba(255,255,255,.05)')}
                      onMouseLeave={e => ((e.currentTarget as HTMLElement).style.background = 'rgba(255,255,255,.025)')}
                    >
                      <div style={{ flex: 1, fontSize: '.71rem', fontWeight: 500, color: '#EEF2F8', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{t.name}</div>
                      <span style={{ fontSize: '.62rem', fontWeight: 700, color: isToday ? '#FBBF24' : isTmr ? 'rgba(251,191,36,.6)' : 'rgba(238,242,248,.28)', whiteSpace: 'nowrap', flexShrink: 0 }}>{label}</span>
                    </div>
                  </a>
                );
              })}
        ...
        {(loading || approvalTasks.length > 0) && (
          <div style={{ background: 'linear-gradient(145deg, rgba(251,191,36,.07) 0%, rgba(251,191,36,.02) 40%, #0B0D1A 70%)', borderRadius: 20, padding: '20px 22px', border: '1px solid rgba(251,191,36,.12)' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
              <div style={{ width: 6, height: 6, borderRadius: '50%', background: '#FBBF24', flexShrink: 0 }} />
              <span style={{ fontSize: '.6rem', fontWeight: 700, color: 'rgba(238,242,248,.22)', textTransform: 'uppercase', letterSpacing: '.12em' }}>Em aprovação</span>
              {!loading && <span style={{ fontSize: '.58rem', fontWeight: 700, padding: '1px 6px', borderRadius: 8, background: 'rgba(251,191,36,.15)', color: '#FBBF24' }}>{approvalTasks.length}</span>}
            </div>
            {loading ? (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>{[1,2].map(i => <Skel key={i} w="100%" h={32} r={8} />)}</div>
            ) : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
                {approvalTasks.slice(0, 5).map(t => (
                  <a key={t.id} href={t.url} target="_blank" rel="noopener noreferrer" style={{ textDecoration: 'none' }}>
                    <div style={{ fontSize: '.71rem', fontWeight: 500, color: '#EEF2F8', padding: '7px 10px', borderRadius: 9, background: 'rgba(251,191,36,.05)', border: '1px solid rgba(251,191,36,.1)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', transition: 'background .15s' }}
        ...
        {/* Atrasadas */}
        {!loading && hasOverdue && (
          <div style={{ background: 'linear-gradient(145deg, rgba(255,107,107,.1) 0%, rgba(255,107,107,.03) 40%, #0B0D1A 70%)', borderRadius: 20, padding: '20px 22px', border: '1px solid rgba(255,107,107,.18)' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
              <div style={{ width: 6, height: 6, borderRadius: '50%', background: '#FF6B6B', flexShrink: 0 }} />
              <span style={{ fontSize: '.6rem', fontWeight: 700, color: 'rgba(238,242,248,.22)', textTransform: 'uppercase', letterSpacing: '.12em' }}>Atrasadas</span>
              <span style={{ fontSize: '.58rem', fontWeight: 700, padding: '1px 6px', borderRadius: 8, background: 'rgba(255,107,107,.15)', color: '#FF6B6B' }}>{overdue.length}</span>
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
              {overdue.slice(0, 4).map(t => (
                <a key={t.id} href={t.url} target="_blank" rel="noopener noreferrer" style={{ textDecoration: 'none' }}>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8, padding: '7px 10px', borderRadius: 9, background: 'rgba(255,107,107,.05)', border: '1px solid rgba(255,107,107,.1)', transition: 'background .15s' }}
                  ...
                  >
                    <span style={{ fontSize: '.71rem', fontWeight: 500, color: '#EEF2F8', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', flex: 1 }}>{t.name}</span>
                    <span style={{ fontSize: '.62rem', fontWeight: 700, color: '#FF6B6B', whiteSpace: 'nowrap', flexShrink: 0 }}>{formatDue(t.due_date)}</span>
                  </div>
                </a>
              ))}
            </div>
        ...
        {/* Atividade recente */}
        <div style={{ background: '#0B0D1A', borderRadius: 20, padding: '20px 22px', border: '1px solid rgba(255,255,255,.05)' }}>
          <div style={{ fontSize: '.6rem', fontWeight: 700, color: 'rgba(238,242,248,.22)', textTransform: 'uppercase', letterSpacing: '.12em', marginBottom: 14 }}>Atividade recente</div>
          {loading ? (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>{[1,2,3].map(i => <Skel key={i} w="100%" h={30} r={6} />)}</div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column' }}>
              {recentActivity.map((t, i) => {
                ...
                return (
                  <div key={t.id} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '7px 0', borderBottom: i < recentActivity.length - 1 ? '1px solid rgba(255,255,255,.04)' : 'none' }}>
                    <div style={{ width: 5, height: 5, borderRadius: '50%', background: st.color, flexShrink: 0 }} />
                    <span style={{ flex: 1, fontSize: '.7rem', color: 'rgba(238,242,248,.55)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{t.name}</span>
                    <span style={{ fontSize: '.59rem', color: 'rgba(238,242,248,.2)', whiteSpace: 'nowrap', flexShrink: 0 }}>{age}</span>
                  </div>
                );
              })}
            </div>
    ...
    if (id === 'tcc-extra') return <>{children}</>;
    return null;
  }
  ...
  return (
    <div style={{ fontFamily: 'Sora, sans-serif' }}>
      ...
      {/* ── GRID LAYOUT (KPIs + Prioridade + Sidebar + Extra) ───────────────── */}
      <GridLayout pageId={`tcc-${config.listId ?? config.name}`} cards={tccCards} renderCard={renderCard} gap={14} />
