...
function NavIcon({ name, size = 15 }: { name: string; size?: number }) {
  ...
  if (name === 'menu')       return <svg {...props}><line x1="3" y1="6" x2="21" y2="6"/><line x1="3" y1="12" x2="21" y2="12"/><line x1="3" y1="18" x2="21" y2="18"/></svg>;
...
export default function Layout({ children }: LayoutProps) {
  ...
  // Close a11y popup on outside click
  useEffect(() => {
      ...
      <div className="mobile-topbar">
        <button
          ...
          style={{
            ...
            color: 'rgba(238,242,248,.7)', padding: 4, display: 'flex',
          }}
        ...
        <Link to="/dashboard" style={{ textDecoration: 'none', display: 'flex', alignItems: 'center', gap: 8 }}>
          <img src="/logo-nova.svg" alt="Nova" style={{ height: 16, width: 'auto' }} />
          <span style={{ fontFamily: 'Sora, sans-serif', fontWeight: 800, fontSize: '.8rem', color: 'var(--nova-text)', letterSpacing: '.06em' }}>
            I MKT
          </span>
        </Link>
      </div>
      ...
      <aside className={`app-sidebar${sidebarOpen ? ' sidebar-open' : ''}`}>
        ...
        <button
          ...
          style={{
            ...
            borderRadius: 8, cursor: 'pointer', color: 'rgba(238,242,248,.5)',
